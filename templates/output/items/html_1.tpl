    ...
    {if $plugin eq 'Newsletter_NewsletterPlugin_NewMembers'}
        <table style="width: 100%; text-align: left;">
            ...
            {foreach from=$objectArray.$plugin item="item" name="loop"}
                <tr>
                    <td>{if $profileAvailable}<div style="font-weight: bold;"><a style="color: #813939; text-decoration: none;" href="{modurl modname="Profile" type="user" func="view" uid=$item.uid lang=$nllang fqurl=true}">{/if}{$item.uname|safehtml}{if $profileAvailable}</a></div>{/if}</td>

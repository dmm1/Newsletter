...
*/
function smarty_function_newsletter_pages($params, &$smarty)
{

	extract($params);
 	
	if (!pnModAvailable('pages')) return;	
	if (!pnModAPILoad('pages', 'user')) return;	
	...
	$table = $pntable['pages'];
    $column = &$pntable['pages_column'];
	$sql = "SELECT $column[pageid],
				   $column[title],
                   $column[content]
            FROM $table
            ORDER BY $column[pageid] DESC
            LIMIT $limit_items";
    $result = $dbconn->Execute($sql);
    
	$data = array();
    for (; !$result->EOF; $result->MoveNext()) {
    	list($pageid, $title, $content) = $result->fields;
    	if($limit_text and strlen($content)>$limit_text){
    		$content = substr($content,0,$limit_text).'..';
    	}
    	$data[] = array('pages_pageid'=>$pageid,
     					'pages_title'=>$title,
